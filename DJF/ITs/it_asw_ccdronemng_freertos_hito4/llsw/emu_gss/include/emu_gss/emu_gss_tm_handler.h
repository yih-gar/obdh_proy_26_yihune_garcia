...
protected:
	...
	struct ccsds_packet_primary_header mTMPacketHeader;
	struct ccsds_pus_tm_df_header mDFHeader;

	const uint8_t *pmNextAppBytes;

public:


	GSSTMHandler(const uint8_t * pTMBytes);

	static uint8_t GetPUSType(const struct tm_mem_descriptor *pTMDescriptor);
	static uint8_t GetPUSSubtype(const struct tm_mem_descriptor *pTMDescriptor);
