...
CEDROOMMessage::CEDROOMMessage() {
	...
	mp_DataPool = NULL;
	signal = 0;
	priority = TEDROOMPriority(0);
	mp_Interface = NULL;
	data = NULL;
	type = TEDROOMMessageType(0);
...
CEDROOMTimingSAP::CEDROOMTimingSAP(CEDROOMComponent *p_Component_,
		...
		m_SynchrSem((unsigned int) 0), m_TimerTask(EDROOMfunTimer,
				p_Component_->m_NomPriority, _EDROOM_TIMING_TASK_STACK)

				, m_WaitComponentSem((unsigned int) 0)
...
CEDROOMComponent::CEDROOMComponent(TEDROOMComponentID componentID,
		...
		m_ComponentID(componentID), m_SynchrMsgPool(&m_SynchrMsgOut), m_ComponentTask(
				...
				pComponentMemory->mp_QueueHeads), EDROOMName("cmp")
...
CEDROOMComponent::CEDROOMComponent(const char *name,
		...
		m_ComponentID(componentID), m_SynchrMsgPool(&m_SynchrMsgOut), m_ComponentTask(
				...
				&m_ComponentTask, &this->m_Mutex, numPri,
				&pComponentMemory->m_QueueNodesPool,
				pComponentMemory->mp_QueueHeads), EDROOMName(name)
